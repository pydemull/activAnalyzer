...
* Kilocalories. For non-SED epochs, MET values are multiplied by BMR expressed in kcal/min when using the Santos-Lozano et al. [-@santos-lozanoActigraphGT3XValidation2013] equations. When using the  Sasaki et al. [-@sasakiValidationComparisonActiGraph2011] and Freedson et al. [-@freedsonCalibrationComputerScience1998] equations, the MET values are multiplied by weight and 1/60. For SED epochs, BMR expressed in kcal/min is directly used;
...
* `wear_time`: total wear time computed using the whole day.
* `wear_time_revised`: total wear time computed using the daily period defined in the app.
* `total_counts_axis1`: total counts for the vertical axis.
* `total_counts_vm`: total counts for the vector magnitude.
* `axis1_per_min`: mean of the counts per minute for the vertical axis (`total_counts_axis1` / 1440).
* `vm_per_min`: mean of the counts per minute for the vector magnitude (`total_counts_vm` / 1440).
