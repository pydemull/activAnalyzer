...
**Period considered to validate a day based on wear time:** from `r params$start_day_analysis` to `r params$end_day_analysis`.

**Minimum number of hours with wear time to validate a day**: `r params$minimum_wear_time_for_analysis` hours.

**Abbreviations:** SED = sedentary, LPA =  light physical activity, VPA = vigorous physical activity, MVPA = moderate-to-vigorous physical activity, PAL = physical activity level.
...
**PROactive scores (for COPD patients):** Step count score = `r compute_pro_actigraph_score(df1_medians[["total_steps"]], metric = "steps")` (daily median of `r df1_medians[["total_steps"]]` steps);  VMU score = `r compute_pro_actigraph_score(df1_medians[["vm_per_min"]], metric = "vm")` (daily median of `r df1_medians[["vm_per_min"]]` counts/min).
...
# Figure
  g_pal <-
    ...
    geom_point(aes(x = 0.25, y = df1_means[["pal"]], color = "Patient's result (daily mean)"), size = 7, shape = 1) +
    geom_point(aes(x = 0.25, y = df1_means[["pal"]], color = "Patient's result (daily mean)"), size = 4, shape = 16) +
    geom_point(aes(x = 0.25, y = df1_means[["pal"]], color = "Patient's result (daily mean)"), size = 7, shape = 3) +
...
# Converting MVPA minutes to MET-hours
  guidelines_status <- 
    ifelse(df1_means[["mets_hours_mvpa"]] < 150/7/60*3, "below the 2020 WHO physical activity guidelines",
                              ifelse(df1_means[["mets_hours_mvpa"]] >  150/7/60*3*2, "above the 2020 WHO physical activity guidelines", "within   the 2020 WHO physical activity guidelines"))
  

# Defining PAL status
  pal_status <- ifelse(df1_means[["pal"]] <1.40, "Insufficient",
                      ifelse(df1_means[["pal"]] <= 1.69, "Sedentary or light activity lifestyle",
                                         ifelse(df1_means[["pal"]] <= 1.99, "Active or moderately active lifestyle",
                                              ifelse(df1_means[["pal"]] <= 2.40, "Vigorous or vigorously active lifestyle", "Likely not sustainable"))))

# ref: http://www.fao.org/3/y5686e/y5686e07.htm#bm07.3

# Ratio MVPA / SED comment
  comment_ratio <- 
    ifelse(df1_means[["ratio_mvpa_sed"]] >= 0.04, "the patient likely has already most of the health benefits from their physical behavior",
