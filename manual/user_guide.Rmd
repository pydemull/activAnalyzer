...
* It computes basal metabolic rate (BMR), based on the sex, age, and weight inputs, and on one of the equations retrieved from the paper by Henry et al. [-@henryBasalMetabolicRate2005]. These equations are shown in Table \@ref(tab:METsEquations).
...
">=70"   ,"female", "10.0 * weight + 577")
  ...
  autofit()
```
When the patient/client provides an "undefined" sex, an equation for females is used. These equations provides BMR in kcal/day, but the app also silently computes BMR in kcal/min to use it in future calculations.
...
Once these new metrics added to the initial dataset, the app summarizes results by day using valid wear time only for near all metrics. The metrics obtained are the followings: 
...
# Results and export
When using the app, results by day and those averaged using valid days are shown in tables. The user can click on specific buttons to export either results by day or results averaged using valid days to .csv files. A last button allows the user to generate a report where all the inputs of the app are recorded, as well as the results that are shown both in tables and figures. Some comments are provided at the end of the document to help positioning the patient/client in relation to normative values.
