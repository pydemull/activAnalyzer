...
* Sasaki et al. [-@sasakiValidationComparisonActiGraph2011] MPA and VPA cut-points in adults: >=2 690 counts/min (MPA) and >=6 167 counts/min (VPA) [Vector magnitude];
* Santos-Lozano et al. [-@santos-lozanoActigraphGT3XValidation2013] MPA and VPA cut-points in adults: >=3 208 counts/min (MPA) and >=8 565 counts/min (VPA) [Vector magnitude];
* Santos-Lozano et al. [-@santos-lozanoActigraphGT3XValidation2013] MPA and VPA cut-points in older adults: >=2 751 counts/min (MPA) and >=9 359 counts/min (VPA) [Vector magnitude]. 
...
">=70"   ,"female", "10.0 * weight + 577")
  ...
  autofit()
```
When the patient/client provides an "undefined" sex, an equation for females is used. These equations provides BMR in kcal/day, but the app silently compute BMR in kcal/min to use it in future calculations.

Once BMR is calculated, the following metrics are added to the initial dataset for each 60-s epoch:
...
* MET-hours related to MPVA, by multipling the MET value by the time (1/60e of an hour), only when the MET value is >= 3.

Once these new metrics added to the initial dataset, the app summarizes results by day using valid wear time only for near all metrics. The metrics obtained are the following: 
...
Then, the app computes the PAL for each day. To do this, total energy expenditure (TEE) is divided by BMR. Total energy expenditure is obtained by summing the kilocalories measured during wear time epochs and the kilocalories likely expended during nonwear time epochs (that is, kilocalories associated to BMR, as it is assumed that the device was mainly removed during sleeping periods if any, periods during which energy expenditure is near of BMR), and by multiplying this sum by 10/9 to take into account the thermic effect of food. Of course, such calculations may conduct to underestimate TEE and PAL if the device was removed during prolonged periods of physical activity. Moreover, even if the device was correctly worn, the estimate of PAL is very approximative since both BMR and kilocalories related to wear time are estimated using method that may not be accurate at the indivudal level.

Finally, the app computes daily averages of the computed metrics using the days considered as valid.


# Sections 4, 5, and 6
Section 4 allows the visualization  in tables of the results by day and averaged using valid days. In section 5, the user can click on specific buttons to export either results by day or results averaged using valid days to .csv files. A last button allows the user to generate a report where all the inputs of the app are recorded, as well as the results that are shown both in tables and figures. Some comments are provided at the end of the document to help positioning the patient/client in relation to normative values.
