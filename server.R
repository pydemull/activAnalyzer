...
server <- function(input, output, session) {
  ...
  ####################################
  # Getting data and marking wear time
  ####################################
  ...
  # Controlling for correct inputs
    observeEvent(input$validate,
                 shinyFeedback::feedbackWarning(
                   "frame_size", 
                   is.numeric(input$frame_size) == FALSE,
    ...
    observeEvent(input$validate,
                 shinyFeedback::feedbackWarning(
                   "allowanceFrame_size", 
                   is.numeric(input$allowanceFrame_size) == FALSE,
                   "Please choose a number >= 0"
                 )
    )
    
  # Creating the reactive dataset
    path <- eventReactive(input$validate, {
      req(input$upload)
      input$upload$datapath
    })
    
    frame_size <- eventReactive(input$validate, {
      input$frame_size
      })
    
    allowanceFrame_size = eventReactive(input$validate, {
      input$allowanceFrame_size
      })
    
    df <- eventReactive(input$validate, {
      
      if ({is.numeric(frame_size()) == TRUE && 
           frame_size() >= 0 && 
           is.numeric(allowanceFrame_size()) == TRUE && 
           allowanceFrame_size() >= 0}) {
      prepare_dataset(data = path(), frame_size = frame_size(), allowanceFrame_size = allowanceFrame_size())
      } else {
      NULL}
      })
    ...
    output$graph <- renderPlot({
     ...
     if ({is.data.frame(df()) == TRUE && 
      ...
      ggplot(data = df(), aes(x = time)) +
        geom_ribbon(aes(ymin = 0, ymax = non_wearing_count / non_wearing_count * max_metric, fill = "Non-wear time"), alpha = 0.5) +
  ...
  #########################################
    ...
    # Getting BMR
      bmr_kcal_d <- reactive({compute_bmr(age = input$age, sex = input$sex, weight = input$weight)})
    
    
    # Getting results corresponding to valid wear time by day (except for total kcal 
    # that also uses non wear time by attributing bmr to these epochs)
      ...
      # Creating a dataset with results by day  
        results_by_day <-
        df_with_computed_metrics %>%
         ...
         summarise(
           ...
           pal = round((total_kcal_awake + bmr_kcal_min * (24*60 - wear_time) + (total_kcal_awake + bmr_kcal_min * (24*60 - wear_time)) * 0.1/0.9) / bmr_kcal_d(), 2)) %>%
         ungroup()
