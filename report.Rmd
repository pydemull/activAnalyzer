 ...
 * Size of the interval with nonzero counts allowed during a nonwear period: `r params$allowanceFrame_size` min.
 * METs equation: `r params$equation_mets `.
 * Axis used for SED categorization: `r params$axis_sed`.
 * Axis used for MVPA categorization: `r params$axis_sed`.
 * SED cut-point: <`r params$sed_cutpoint` counts/min.
 * MPA cut-point: >=`r params$mpa_cutpoint` counts/min.
 * VPA cut-point: >=`r params$vpa_cutpoint` counts/min.
...
g_pal <-
  ...
  geom_point(aes(x = 0.5, y = df[["pal"]], color = "Result averaged over valid days"), size = 6, shape = 1) +
  geom_point(aes(x = 0.5, y = df[["pal"]], color = "Result averaged over valid days"), size = 3, shape = 16) +
  geom_point(aes(x = 0.5, y = df[["pal"]], color = "Result averaged over valid days"), size = 6, shape = 3) +
...
g_steps <-
  ...
  geom_point(aes(x = 0.5, y = df[["total_steps"]]), color = "red", size = 6, shape = 3) +
...
g_mvpa <-
  ...
  theme_bw() +
  coord_flip(expand = FALSE) +
  labs(x = "", y = "", color = "2021 WHO guidelines") +
  theme(axis.ticks = element_blank(),
        axis.text.y = element_blank(),
        legend.position = "right",
        legend.title = element_text(face = "bold" , size = 10),
...
guidelines_status <- ifelse(df[["mets_hours_mvpa"]] < 150/7/60*3, "below the 2021 WHO physical activity guidelines",
                            ifelse(df[["mets_hours_mvpa"]] >  150/7/60*3*2, "above the 2021 WHO physical activity guidelines", "within the 2021 WHO physical activity guidelines"))
...
library(patchwork)
g_pal / g_steps / g_mvpa / g_sed + plot_layout(heights = c(2, 2, 2, 2)) & theme(legend.justification = "left")
```
...
df2 %>%
  ...
  scale_fill_manual(values = c("#00BE6C", "#FC717F")) +
  theme_bw() + 
...
Based on the estimated PAL and the [Food and Agriculture Organization of the United Nations (FAO)](http://www.fao.org/3/y5686e/y5686e.pdf), the activity lifestyle can be qualified as "**`r pal_status`**" (this is a rough estimate because both basal metabolic rate and total energy expenditure were estimated from methods and equations that may not be accurate for a given individual).

**Step count**

Regarding the daily step number, the expected values proposed for the different categories of populations and that would correspond to reaching the 2021 World Health Organization (WHO) physical activity guidelines (cf. Summary, Graphical View) suggest the inclusion of both steps related to activities performed in the course of daily living AND at least 3000 steps taken at moderate intensity or higher. Importantly, the expected values were defined based on scientific literature that used classical pedometers. If the ActiGraph model used was a GT3X or a more recent model, the result obtained is likely to underestimate or overestimate the actual daily step count if the normal filter or the lower frequency extension filter, respectively, was used to generate activity counts.
