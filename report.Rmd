...
**Wear time:** Based on `r params$axis_weartime`, interval of `r params$frame_size` min for nonwear time detection, interval of `r params$allowanceFrame_size` min with nonzero counts allowed during a nonwear period | **MET equation:** `r params$equation_mets` | **Axis used for PA categorization:** `r params$axis_mvpa` for MVPA | **Cut-points:** <`r params$sed_cutpoint` counts/min for SED, $\geqslant$ `r params$mpa_cutpoint` counts/min for MPA, $\geqslant$ `r params$vpa_cutpoint` counts/min for VPA.
...
# Ratio MVPA / SED comment
  comment_ratio <- 
    ifelse(df[["ratio_mvpa_sed"]] >= 0.04, "the patient has already most of the health benefits from their physical behavior",
