...
# hazard ratio (Ekelund et al., 2020)
   ...
   model_mvpa <- loess(y ~ x, data = mvpa_lines %>% filter(line == "mid"), 
                       control = loess.control(surface = "direct"))
     
   model_mvpa_up <- loess(y ~ x, data = mvpa_lines %>% filter(line == "up"), 
                       control = loess.control(surface = "direct")) 
   
   model_mvpa_low <- loess(y ~ x, data = mvpa_lines %>% filter(line == "low"), 
                       control = loess.control(surface = "direct"))
   ...
   grid_mvpa <-
     ...
     spread_predictions(model_mvpa, model_mvpa_up, model_mvpa_low) %>%
     rename(mid = model_mvpa, up = model_mvpa_up, low = model_mvpa_low)

# Getting data for plotting patient's result  
  
   score_mvpa <- data.frame(x = df[["minutes_MVPA"]]) %>% add_predictions(model_mvpa)
...
# Plot
 ...
 g_mvpa <-
   ggplot() +
   geom_ribbon(data = grid_mvpa, aes(x = x, y = up, 
   ...
   geom_point(data = score_mvpa, aes(x = x, y = pred), color = "red", size = 6, shape = 1) +
   geom_point(data = score_mvpa, aes(x = x, y = pred), color = "red", size = 3, shape = 16) +
   geom_point(data = score_mvpa, aes(x = x, y = pred), color = "red", size = 6, shape = 3) +
   scale_y_continuous(trans = log2_trans()) +
   scale_x_continuous(limits = c(-0.5, 100), breaks = seq(0, 100, 25)) +
   theme_bw() +
   coord_cartesian(xlim = c(0, 100), ylim = c(0.25, 1), expand = FALSE) +
   labs(title = "Daily MVPA minutes",
        x = "",
        y = NULL) +
   theme(axis.ticks = element_blank(),
         axis.text.x = element_text(size = 13),
...
# hazard ratio (Ekelund et al., 2020)
   ...
   model_sed <- loess(y ~ x, data = sed_lines %>% filter(line == "mid"), 
                       control = loess.control(surface = "direct"))
     
   model_sed_up <- loess(y ~ x, data = sed_lines %>% filter(line == "up"), 
                       control = loess.control(surface = "direct")) 
   
   model_sed_low <- loess(y ~ x, data = sed_lines %>% filter(line == "low"), 
                       control = loess.control(surface = "direct"))
   ...
   grid_sed <-
     ...
     spread_predictions(model_sed, model_sed_up, model_sed_low) %>%
     rename(mid = model_sed, up = model_sed_up, low = model_sed_low)

# Getting data for plotting patient's result  
  
   score_sed <- data.frame(x = df[["minutes_SED"]] / 60) %>% add_predictions(model_sed)

# Plot

 g_sed <-
   ggplot() +
   geom_ribbon(data = grid_sed, aes(x = x, y = up, 
                                       ymin = low, ymax = up), fill = "grey95") +
   geom_ribbon(data = grid_sed %>% filter(x < 6), aes(x = x, y = up, 
                                       ymin = rep(0, 2), ymax = up), fill = "grey95") +
   ...
   geom_point(data = score_sed, aes(x = x, y = pred), color = "red", size = 6, shape = 1) +
   geom_point(data = score_sed, aes(x = x, y = pred), color = "red", size = 3, shape = 16) +
   geom_point(data = score_sed, aes(x = x, y = pred), color = "red", size = 6, shape = 3) +
   scale_y_continuous(trans = log2_trans()) +
   scale_x_continuous(limits = c(4, 14), breaks = seq(4, 14, 2)) +
   coord_cartesian(xlim = c(4, 14), ylim = c(0.35, 5), expand = FALSE) +
   labs(title = "Daily SED hours",
        x = "",
        y = NULL) +
   theme_bw() +
   theme(axis.ticks = element_blank(),
...
# hazard ratio (Ekelund et al., 2020)
   ...
   model_ratio <- loess(y ~ x, data = ratio_lines %>% filter(line == "mid"), 
                       control = loess.control(surface = "direct"))
     
   model_ratio_up <- loess(y ~ x, data = ratio_lines %>% filter(line == "up"), 
                       control = loess.control(surface = "direct")) 
   
   model_ratio_low <- loess(y ~ x, data = ratio_lines %>% filter(line == "low"), 
                       control = loess.control(surface = "direct"))
   ...
   grid_ratio <-
     ...
     spread_predictions(model_ratio, model_ratio_up, model_ratio_low) %>%
     rename(mid = model_ratio, up = model_ratio_up, low = model_ratio_low)

# Getting data for plotting patient's result  
  
   score_ratio <- data.frame(x = df[["ratio_mvpa_sed"]]) %>% add_predictions(model_ratio)
...
# Plot
 ...
 g_ratio <-
   ...
   geom_point(data = score_ratio, aes(x = x, y = pred), color = "red", size = 6, shape = 1) +
   geom_point(data = score_ratio, aes(x = x, y = pred), color = "red", size = 3, shape = 16) +
   geom_point(data = score_ratio, aes(x = x, y = pred), color = "red", size = 6, shape = 3) +
   scale_x_continuous(limits = c(0, 0.4), breaks = seq(0, 0.4, 0.1)) +
   coord_cartesian(xlim = c(0, 0.4), ylim = c(0.1, 1.3), expand = FALSE) +
   labs(title = "Daily MVPA / SED ratio",
        x = "",
        y = NULL) +
   theme_bw() +
   theme(axis.ticks = element_blank(),
...
library(patchwork)
g_pal / g_steps / (g_mvpa | g_sed | g_ratio) /  g_week + plot_layout(heights = c(0.8, 0.7, 1.5, 4)) & theme(legend.justification = "center")
```
